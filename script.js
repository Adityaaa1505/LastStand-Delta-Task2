...
function homeScreen() {
    ...
    ctx.strokeRect(100, 590, canvas.width - 200, 75)
    ctx.fillText("Play", 680, 645)
    ctx.font = "40px Ariel"
    ...
    ctx.fillText("Made with ❤️ by Aditya", 630, canvas.height - 10)

    //If Play Button Clicked
    document.addEventListener("mousedown", function () {
        if (100 < mouseX && mouseX < canvas.width - 200 && 590 < mouseY && mouseY < 665) {
            game.active = true
            return
        }
    });
